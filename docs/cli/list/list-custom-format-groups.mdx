...
For a specific service, this command prints detailed information about each custom format group from
the TRaSH Guides. Each group is displayed as a panel containing a table of its custom formats (with
name, trash ID, required/default flags) and associated quality profiles. Use the Trash ID values
with the `custom_format_groups` property in your configuration YAML file.
