...
For more details about how includes and merging work, see the [Merge Behavior](#behavior) section
below.
...
[appdata]: ../../guide/file-structure.mdx#include-directory
...
Utilize a [Configuration Templates][templates] repository to include YAML files. Template includes
reference entries in a repository's `includes.json`.

[templates]: https://github.com/recyclarr/config-templates

:::warning

The official config-templates repository no longer provides include templates as of v8. This
directive is only useful with [custom resource providers][providers] that supply their own
`includes.json`.
...
```yml
include:
  # Includes YAML templates by ID from a resource provider's includes.json
  - template: my-custom-quality-profile
  - template: my-custom-formats
```
...
- **Include Template**<br/>
  A type of template that may *only* be included. The official config-templates repository no longer
  provides include templates; this concept applies only to custom resource providers.
...
- **Include Directive**<br/>
  A specific method of including a YAML file (e.g. `config: myfile.yml`). See the [include
  directives](#include) section above for all available types.
...
A join involves two *sequences* of *mappings*. Based on some key, such as a `trash_id` or `name`
property, two sequences are joined together. If sequence `B` has a key that matches any key in
sequence `A`, then those two mappings are *combined* (see [Union](#union)). For mappings in sequence
`B` that have *no match*, the behavior is effectively the same as [Add](#add).
...
Guide-backed profiles can also be joined by `trash_id`:
...
| `media_management`          | Mapping   | Union               |

### Quality Definition {#merge-quality-definition}

| Property Name     | Node Type | Merge Operation |
|-------------------|-----------|-----------------|
| `type`            | Scalar    | Replace         |
| `preferred_ratio` | Scalar    | Replace         |
| `qualities`       | Sequence  | Join            |

Items in `qualities` are joined by `name`. Individual properties (`min`, `max`, `preferred`) use
Replace.

### Quality Profiles {#merge-quality-profiles}

Items in `quality_profiles` are joined by both `trash_id` and `name` when `trash_id` is present,
otherwise by `name` alone. Profiles sharing a `trash_id` but with different names are kept as
separate entries, not merged.
...
| `except`          | Sequence  | Add             |
| `except_patterns` | Sequence  | Add             |

### Custom Format Groups {#merge-custom-format-groups}

Items in `custom_format_groups` are joined by `trash_id`.

| Property Name      | Node Type | Merge Operation |
|--------------------|-----------|-----------------|
| `assign_scores_to` | Sequence  | Replace         |
| `select`           | Sequence  | Replace         |
| `select_all`       | Scalar    | Replace         |
| `exclude`          | Sequence  | Replace         |

### Media Naming {#merge-media-naming}
...
### Media Management {#merge-media-management}

| Property Name         | Node Type | Merge Operation |
|-----------------------|-----------|-----------------|
| `propers_and_repacks` | Scalar    | Replace         |

### Unsupported Properties {#merge-unsupported}
...
profile `trash_id` if present, otherwise `name`, as the *key*.
...
### Custom Format Groups {#detail-cfg}

The `assign_scores_to`, `select`, and `exclude` properties use Replace rather than Add or Join. This
provides simpler override semantics: when you specify these properties in your root config, they
completely replace any values from included YAML for that group. This makes it clear which profiles
receive scores and which CFs are included or excluded without needing to mentally combine multiple
lists.

## Tips & Information {#tips}

### Include File Structure {#include-file-structure}
