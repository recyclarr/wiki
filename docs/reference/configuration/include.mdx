...
- **Include Template**<br/>
  A type of template that may *only* be included. The official config-templates repository no
  longer provides include templates; this concept applies only to custom resource providers.
...
The `assign_scores_to` and `select` properties use Replace rather than Add or Join. This provides
simpler override semantics: when you specify these properties in your root config, they completely
replace any values from included YAML for that group. This makes it clear which profiles receive
scores and which optional CFs are selected without needing to mentally combine multiple lists.
