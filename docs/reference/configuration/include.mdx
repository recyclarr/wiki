...
Items in `quality_profiles` are joined by `trash_id` if present, otherwise by `name`. This allows
guide-backed profiles to merge correctly even if the guide renames a profile.
