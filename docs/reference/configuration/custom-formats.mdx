...
**Optional.** *Default: No entry-level default is applied*

A default score used by every profile under [`assign_scores_to`](#assign-scores-to) that does not
define its own `score`. Useful when you want to apply the same score across many profiles without
repeating it on each one.

Score resolution for a profile proceeds in this order, stopping at the first match:

1. The per-profile [`score`](#qp-score) under `assign_scores_to`
2. This entry-level `score`
3. The score from the profile's [`score_set`][score-set], if configured
4. The score from the guide

[score-set]: ./quality-profiles.mdx#score-set

Example using an entry-level default with a single override:
  ...
  - trash_ids:
    ...
    assign_scores_to:
      ...
      - name: REMUX-MULTi-VF-HD
        score: 100 # overrides the entry-level 0
```
...
are taken from the guide by default, with options to override at the entry or per-profile level.
Profiles can be referenced by `name` or `trash_id`. Each entry must use exactly one of these
properties. Specifying both produces a validation error.
...
**Optional.** *Default: Use the entry-level [`score`](#cf-score), or scores from the guide*
...
When set, this value overrides the entry-level [`score`](#cf-score) for this profile.
...
Without `assign_scores_to`, a group only assigns scores to profiles in its `include` list — not
every profile in your configuration. If your profile isn't included by the group, it is silently
skipped.

If CFs from a group aren't appearing in your profile, check which profiles the group includes (use
[`list custom-format-groups`][listcfgroups] to inspect this). When your profile isn't included, add
an explicit `assign_scores_to` entry to target it.
