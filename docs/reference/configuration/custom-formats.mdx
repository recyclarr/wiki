...
`trash_id` (for guide-backed profiles) or `name` (for custom profiles). Each entry must use one of
these properties, not both.

When omitted, scores are assigned to guide-backed profiles in your configuration that the group
includes. Each group specifies compatible profiles; for example, HDR groups include only UHD
profiles. Auto-sync only targets guide-backed profiles; to assign group scores to user-defined
profiles, specify them explicitly with `name`.
...
quality profile resources, otherwise a validation error is reported.

##### `name` {#cfg-ast-name}

**Conditionally Required.** Required if `trash_id` is not specified.

The name of a quality profile to assign group scores to. Use this for custom (non-guide-backed)
profiles. Referencing a guide-backed profile by name produces a validation error; use `trash_id`
instead. If the named profile does not exist in your configuration or in the remote service, a
warning is produced during sync.
...
Selecting a CF that is already a default produces a warning since it has no effect.

#### `exclude` {#cfg-exclude}

**Optional.** *Default: No default CFs are excluded*

A list of CF trash IDs to remove from the group's defaults. Use this to opt-out of specific default
CFs you don't want synced. Required CFs cannot be excluded.

Excluding a non-default or required CF produces a warning since neither has any effect.

A CF cannot appear in both `select` and `exclude`. The resulting set of CFs is: required CFs, plus
default CFs minus `exclude`, plus `select`.
