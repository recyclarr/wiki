...
The name of a quality profile defined in your [`quality_profiles`][qps] configuration. Use this to
assign group scores to custom (non-guide-backed) profiles. Referencing a guide-backed profile by
name produces a validation error; use `trash_id` instead. The profile must exist in your
configuration's `quality_profiles` list.
