...
A list of groups to explicitly sync. Use this to opt-in to non-default groups or customize a
default group's behavior (e.g., specify `select` to change which CFs sync).
...
One or more quality profiles to assign the group's CF scores to. Only accepts `trash_id` references
to [guide-backed quality profiles][guide-qp], not profile names.
...
profiles. User-defined profiles (those with only a `name`) do not receive scores from groups.

Referencing a profile not in the group's include list results in a validation error.
