...
each entry sharing a `trash_id` must have a distinct `name`. At most one entry per `trash_id` may
omit `name` (inheriting it from the guide); all others must specify explicit names. Name resolution
(guide name inheritance) happens before duplicate detection.
...
Two validation checks run before syncing:

- Two entries with the same `trash_id` must have distinct `name` values. If duplicates are found,
  both entries are skipped with an error.
- Two profiles cannot resolve to the same name regardless of `trash_id`. If a name collision is
  detected, both entries are skipped with an error.
