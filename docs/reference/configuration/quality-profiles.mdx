...
detected, all entries with that name are skipped with an error. This applies after name resolution
(guide name inheritance), so two entries sharing a `trash_id` must have distinct `name` values to
avoid colliding.
