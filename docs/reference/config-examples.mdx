...
## Renaming a guide-backed quality profile {#rename-guide-profile}

Scenario: *I want to use a TRaSH Guide quality profile but with my own custom name instead of the
guide's default name.*

Solution:

When using `trash_id` to sync a guide-backed profile, specify `name` to override the guide's
default:
  ...
  main:
    ...
    quality_profiles:
      - trash_id: 9a0e43cb7a892e3e9c4f2f5c5e0dcf8e
        name: My Movies - High Quality
```

Because Recyclarr tracks the profile by `trash_id` (not by name), you can change the name at any
time. Simply update `name` in your YAML and run sync - Recyclarr will rename the existing profile
rather than creating a new one.

This is different from profiles without `trash_id`, where the name is the identifier and changing it
would target a different profile entirely.
...
documented on [the File Structure page](../guide/file-structure.mdx#default-yaml). What would that
