...
Each entry must have a distinct `name`. At most one entry per `trash_id` may omit `name` to inherit
the guide's default; all others must specify explicit names. See [Limitations][qp-limits] for rename
constraints when modifying profile variants.
