...
Use [`reset_unmatched_scores`][rus] when you want Recyclarr to have exclusive control over CF scores
in a profile. This is enabled by default in all official templates:

[rus]: ./configuration/quality-profiles.mdx#reset-unmatched-scores
...
Use the same `trash_id` multiple times to create [profile variants][variants] with different
settings. This is useful when you want the same quality structure but with different behavior, such
as different upgrade limits or language-specific libraries.
...
Each variant must have a distinct [`name`][qp-name]. See [Renaming Profile Variants][qp-limits] for
constraints when modifying variants.

[variants]: ./configuration/quality-profiles.mdx#trash-id
[qp-name]: ./configuration/quality-profiles.mdx#name
