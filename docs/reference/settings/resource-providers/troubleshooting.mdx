...
### Repository names must be unique {/*#repository-names-must-be-unique*/}
...
### Invalid characters in names {/*#invalid-characters-in-names*/}
...
### Service required for custom-formats {/*#service-required-for-custom-formats*/}
...
### Only one replace_default per type {/*#only-one-replace_default-per-type*/}
...
Recyclarr auto-detects the default branch by trying `master` then `main`. If your repository uses a
different branch name (e.g., `develop`, `stable`), you must specify it explicitly:

```yml
# Wrong - repository uses 'develop' which isn't auto-detected
- name: my-guides
  type: trash-guides
  clone_url: https://github.com/yourname/guides.git

# Correct - explicitly specify non-standard branch names
- name: my-guides
  type: trash-guides
  clone_url: https://github.com/yourname/guides.git
  reference: develop
...
### Git repository access issues {/*#git-repository-access-issues*/}
...
### Repository structure not recognized {/*#repository-structure-not-recognized*/}
...
## Precedence Issues {/*#precedence-issues*/}

### Which resource gets used when IDs collide? {/*#which-resource-gets-used-when-ids-collide*/}
...
### Custom format not overriding official {/*#custom-format-not-overriding-official*/}
...
### Template not overriding official {/*#template-not-overriding-official*/}
...
## Local Directory Issues {/*#local-directory-issues*/}
...
### Custom formats not loading from local directory {/*#custom-formats-not-loading-from-local-directory*/}
...
### Check Recyclarr logs {/*#check-recyclarr-logs*/}
...
### Verify JSON files manually {/*#verify-json-files-manually*/}
