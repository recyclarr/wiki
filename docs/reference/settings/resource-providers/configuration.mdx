...
Maximum size of the `.git` directory before Recyclarr rebuilds the cache. When the directory exceeds
this limit, Recyclarr deletes it and performs a fresh shallow clone. Accepts a number followed by a
unit: `KB`, `MB`, or `GB`.

Set to `0MB` to disable automatic cleanup entirely.

If the fresh clone still exceeds the limit, Recyclarr logs a warning suggesting you raise the
value. Rebuild failures are best-effort and don't interrupt the sync.
