...
Maximum size of the `.git` directory before Recyclarr runs automatic maintenance after a fetch. When
the directory exceeds this limit, Recyclarr trims shallow history, expires reflogs, and runs garbage
collection to reclaim space. Accepts a number followed by a unit: `KB`, `MB`, or `GB`.

Set to `0MB` to disable automatic cleanup entirely.

This is best-effort: if maintenance fails, the sync continues normally and a warning is logged.
...
When omitted, Recyclarr auto-detects the default branch by trying `master` then `main`. For
`config-templates` providers, Recyclarr first tries `v{major}` (matching your Recyclarr version) to
