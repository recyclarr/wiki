...
description: Configure custom data sources for formats and templates
---
...
For property details, see the [configuration reference][config-ref].

[config-ref]: ./configuration.mdx
...
## When You'd Use This {/*#when-youd-use-this*/}
...
### Settings vs Configuration {/*#settings-vs-configuration*/}
...
When multiple providers have resources with the same identification key:
...
- **Different keys are additive**: Resources with unique keys from all providers are combined

This means your custom resources can override official ones by using the same identification key.

### Resource Identification Keys {/*#resource-identification-keys*/}

Different resource types use different fields to determine uniqueness when merging across providers.
All comparisons are case-insensitive.

| Resource Type    | Identification Key | Example                               |
|------------------|--------------------|---------------------------------------|
| Custom Formats   | `trash_id` field   | `ae4cfaa9283a4f2150ac3da08e388723`    |
| Quality Profiles | `trash_id` field   | `d1d67249d3890e49bc12e275d989a7e9`    |
| Quality Sizes    | `type` field       | `series`, `movie`                     |
| Media Naming     | Dictionary key     | `default`, `plex-imdb`                |
| Config Templates | `id` field         | `hd-bluray-web`                       |
| Config Includes  | `id` field         | `radarr-custom-formats-hd-bluray-web` |

See [Override Examples](./override-examples.mdx) for practical examples of extending and replacing
resources.

## Quick Start: Adding Local Custom Formats {/*#quick-start-adding-local-custom-formats*/}
...
For more details on the JSON structure and available fields, see [Custom Format JSON
Structure](./trash-guides-structure.mdx#custom-format-json-structure).
...
resource_providers:
  - name: my-language-cfs
    type: custom-formats
    path: /home/user/custom-formats/radarr  # Or use relative: custom-formats/radarr
    service: radarr
```
...
### Adding Supplemental Custom Formats {/*#adding-supplemental-custom-formats*/}
...
### Adding Supplemental Git Repositories {/*#adding-supplemental-git-repositories*/}
...
### Replacing Official Sources {/*#replacing-official-sources*/}
...
### Using a Specific Version {/*#using-a-specific-version*/}
...
### Multiple Sources Combined {/*#multiple-sources-combined*/}
...
## Overriding Official Content {/*#overriding-official-content*/}

You can override specific official content by providing your own version with the same ID.

### Overriding a Custom Format {/*#overriding-a-custom-format*/}
...
### Overriding a Config Template {/*#overriding-a-config-template*/}
...
- Auto-detection handles `master` and `main` automatically; specify `reference` only for pinning or
...
Never edit files in repositories managed by Recyclarr (in the resources directory). Local changes
will be destroyed when repositories are updated. Always maintain your customizations in separate
locations.
