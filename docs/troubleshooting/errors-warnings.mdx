...
example](/yaml/configuration-examples.mdx#yaml-structure) showing multiple configuration files
and their structure.
...
directly on a host machine). The [Docker](installation/docker.mdx) page has troubleshooting steps as
well.
