...
- **`add`** — Opt-in to non-default groups, or customize which CFs sync with `select`,
  `select_all`, and `exclude`
...
### Selecting All CFs in a Group {/*#pattern-select-all*/}
...
[`select_all`][cfg-select-all] to include all non-required CFs at once:
...
[cfg-select-all]: ../reference/configuration/custom-formats.mdx#cfg-select-all
