...
to the [`config` include directive][include-config] were rooted in
...
matching: if a custom format is in the cache, Recyclarr trusts that cached ID regardless of name
changes in the service.

To adopt manually-created custom formats into Recyclarr management, use the cache rebuild command:

```bash
recyclarr cache rebuild --adopt
```

This adds existing custom formats (matched by name) to the cache. After adoption, Recyclarr will
...
List commands display formatted tables with category groupings and helpful instructions. Use
`--raw` for machine-readable TSV output suitable for scripting.
