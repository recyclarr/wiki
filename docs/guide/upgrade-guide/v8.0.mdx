...
3. Run [`recyclarr state repair --adopt`][state-repair]. This tells Recyclarr to recognize the
   existing profile as the guide-backed profile going forward.
...
After running `recyclarr state repair --adopt` (step 3), the profile is adopted. You can then remove
`name` to let the guide name take over, or keep it to preserve your current name.
...
[state-repair]: ../../cli/state/repair.mdx
