...
the `base_url` changed. To recover, run the [cache rebuild][cache-rebuild] command:
...
by name. After rebuilding, Recyclarr will recognize the CFs it previously created and resume
managing them normally.
