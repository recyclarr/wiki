...
Media naming uses **dictionary keys** (like `default`, `no-episode-title`) for identification.
...
```json
{
  ...
  "series": {
    ...
    "plex": "{Series TitleYear} {tvdb-{TvdbId}}"
  },
  "episodes": {
    "standard": {
...
- `plex` and `my-compact` are new keys that extend the available options
...
```json
{
  ...
  "file": {
    "default": "{Movie CleanTitle} ({Release Year}) {[Quality Full]}",
...
To replace the official `radarr-cf-remux-web-1080p` include with your own version:
...
[
  {
    "id": "radarr-cf-remux-web-1080p",
    "template": "radarr/includes/cf-remux-web-1080p.yml"
  }
]
```
...
To replace the official `radarr-sqp-1-1080p` template:
...
[
  {
    "id": "radarr-sqp-1-1080p",
    "template": "radarr/templates/sqp-1-1080p.yml"
  }
]
```
