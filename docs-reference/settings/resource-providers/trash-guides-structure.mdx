...
Media naming files define file and folder naming patterns:

```json
{
  "trash_id": "your-naming-id",
  "file": "{Movie Title} ({Release Year}) {Quality Full}",
  "folder": "{Movie Title} ({Release Year})"
}
```

The exact fields depend on whether it's for Radarr or Sonarr, as they have different naming token
systems.
